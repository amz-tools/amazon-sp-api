...
class SellingPartner {
  ...
  async _getReport(req_params, report_id) {
    ...
    if (res.processingStatus === "DONE") {
      ...
      if (
        !req_params.cancel_after ||
        req_params.cancel_after > req_params.tries
      ) {
  ...
  async callAPI(req_params) {
    ...
    req_params.timeouts = Object.assign(
      {},
    ...
    if (json_res.errors?.length) {
      ...
      if (res.statusCode === 403) {
        if (
          ...
          this._options.auto_request_tokens
        ) {
   ...
   *
   * @param {*} req_params
   * @returns ReadableStream oStream readable stream
   */
  async downloadStream(details) {
    ...
    if (res.statusCode !== 200) {
      ...
      // In order to be consistent with return type (a stream), we throw an error here
    ...
    return details.compressionAlgorithm ? res.pipe(zlib.createGunzip()) : res;
  }
   ...
   * @throws {*} CustomError
   * resP is a promise that resolves to the response object (see the example below)
   * Usage:
   ...
   */
  async uploadStream(details, iStream, contentLength, contentType) {
    this._validateDocumentDetails(details);
    let { req, resP } = await this._request.streamUpload({
      url: details.url,
      method: "PUT",
      headers: {
  ...
  async downloadReportStream(req_params) {
    ...
    return await this.downloadStream(report_document);
  }
