...
class SellingPartner {
  ...
  //   version_fallback:true|false // Optional: Whether or not the client should try to use an older version of an endpoint if the operation is not defined for the desired version. Default is true
  // },
  // endpoints_versions:{}, // Optional: Defines the version to use for an endpoint as key/value pairs (.i.e. "reports":"2021-06-30"). Default is using the first (meaning the oldest) version for every endpoint
  ...
  // * restricted_data_token: Optional, a token received from a "createRestrictedDataToken" operation
  // * options:{ // Optional
  // Optional, the version of the endpoint of the operation. If provided, possible endpoint version setting in constructor config object will be ignored.
  // Default will be the oldest possible version for the operation or the newest possible version if "latest" is set for the endpoint
