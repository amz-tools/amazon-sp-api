...
class SellingPartner {
  ...
  _validateAndGetVersion(operation, options){
    let endpoint = operations[operation];
    if (!options || !options.version){
      // if (latest)
      // Iterate through all endpoint versions if no version is given to find the oldest version that supports the operation
      return endpoints[endpoint].__versions.find((__version) => {
        return endpoints[endpoint][__version][operation];
      });
    } else if (!endpoints[endpoint].__versions.includes(options.version)){
      throw new CustomError({
        code:'INVALID_VERSION',
        message:'Invalid version "' + options.version + '" for endpoint "' + endpoint + '" and operation "' + operation + '". Should be one of: "' + endpoints[endpoint].__versions.join('","') + '"'
      });
    } else if (!endpoints[endpoint][options.version][operation]){
      throw new CustomError({
        code:'OPERATION_NOT_FOUND_FOR_VERSION',
        message:'Operation "' + operation + '" not found for specified version "' + options.version + '"'
      });
    }
  ...
  // * options:{ // Optional
  //   version:'<OPERATION_ENDPOINT_VERSION>', // Optional, the version of the endpoint of the operation. Default will be the oldest possible version for the operation or the newest possible version if "latest" is set for the endpoint
  //   raw_result:true|false // Optional, whether or not the client should return the "raw" result without formatting or error checking, see examples for more information. Default is false
  // }
  async callAPI(req_params){
    let operation = req_params.operation;
    this._validateOperation(operation);
    let version = this._validateAndGetVersion(operation, req_params.options);
