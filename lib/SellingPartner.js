...
class SellingPartner {
  ...
  // If scope is provided it should be one of: ['sellingpartnerapi::notifications', 'sellingpartnerapi::migration']
  ...
  async callAPI(req_params){
    ...
    if (this._options.auto_request_tokens){
      if ((!this._access_token && !scope) || (scope && !this._grantless_tokens[scope])){
    ...
    if ((!this._access_token && !scope) || (scope && !this._grantless_tokens[scope]) || !this._role_credentials){
      throw new CustomError({
