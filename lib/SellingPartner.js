...
class SellingPartner {
  ...
  async _createReport({ date_range, report_type, options }) {
    const response = await this.callAPI({
      operation: 'reports.createReport',
      body: {
        reportType: report_type,
        dataStartTime: date_range.start,
        dataEndTime: date_range.end,
        ...options
      }
    });
  ...
  async _getReport(report_id, report_type) {
    return new Promise(async (resolve, reject) => {
      ...
      if (response.processingStatus === 'DONE') {
        console.log(`REPORTING ${report_type} => ${response.processingStatus}`);
        resolve(response.reportDocumentId);
      } else if (
        response.processingStatus === 'CANCELLED' ||
        response.processingStatus === 'FATAL'
      ) {
        console.log(`REPORTING ${report_type} => ${response.processingStatus}`);
        reject(
          new CustomError({
            code: `REPORT_${response.processingStatus}`,
            message: 'Something went wrong while processing report'
          })
        );
      } else {
        console.log(
          `processing status of ${report_type}: ${response.processingStatus}`
        );
        setTimeout(() => {
          resolve(this._getReport(report_id, report_type));
        }, 10000);
      }
    });
  }

  async _getReportDocumentOptions(report_document_id) {
    let response = await this.callAPI({
  ...
  async downloadReport(req_params) {
    ...
    const report_document_id = await this._getReport(report_id, req_params.report_type);
    const report_document_options = await this._getReportDocumentOptions(report_document_id);
    return await this.download(report_document_options, req_params.report_options || {});
  }
