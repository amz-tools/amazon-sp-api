...
let utils = {
  ...
  checkAndEncodeParams: (req_params, requirements) => {
    if (!req_params || !req_params.path) {
      throw new CustomError({
        code: "NO_PATH_FOUND",
        message: `Please provide the following path parameters: ${Object.keys(
          requirements.path
        ).join(",")}`
      });
    }
  ...
  doubleEncodeURICompenent: (str, key_is_array) => {
    return key_is_array
      ? utils.encodeURIComponent(
          utils.encodeURIComponent(str).replace(/%2C/g, ",")
        )
  ...
  printWarning: {
    ...
    SANDBOX_ONLY: (operation) => {
      console.warn(
        `SANDBOX ONLY! The operation '${operation}' is only available for sandbox endpoints.`
      );
    }
  },
