...
// returns a Promise on steroids by exposing the functions resolve() and reject().
// The usage scenario demands 2 actors: the creator (the callee) and the consumer (the caller).
// The advantage over the regular Promise is that the creator can resolve or reject the promise
// without encapsulating the code that actually resolves or rejects the promise.
// The drawback is that the consumer can resolve or reject the promise as well, which is not the
// intended use case.
// rif: https://gist.github.com/stefanmaric/895f51652060a820e2ee7f164af87948
const defer = () => {
  const bag = {}
...
class Request {
   ...
   * streamUpload
   * @param {*} req_options
   * @returns Promise({req, resP: Promise({body, chunks, statusCode, headers})})
   * resP is a promise that resolves to the response object
   * Usage:
   * let iStream = require('fs').createReadStream(feed.file, 'utf-8');
   ...
   * },
   * }
   * let {req, res} = await request.stream(req_options);
   * let uStream = iStream.pipe(req);
   * let {body, chunks, statusCode, headers} = await res;
   * // check status code and use body for the response or raise an error
   */
    streamUpload(req_options) {
      return new Promise((resolve, reject) => {
        // check if method is PUT
        if (req_options.method != "PUT") {
          reject(new Error("Method must be PUT"));
        }
        let url = new URL(req_options.url);
        let options = {
          method: req_options.method,
     ...
     *      }
     * );
     * const streamOnTarget = (targetFile) => (stream) => {
     *  const ostream = require('fs').createWriteStream(targetFile);
     *  return new Promise((resolve, reject) => {
     *    console.log("in promise");
     *    stream.pipe(ostream).on('end', resolve).on('error', reject);
     *    ostream.on('end', resolve);
     *    stream.on('end',resolve);
     *  });
     * }
     * targetFile = '/path/to/targetFile';
     * const streamOnTargetFile = streamOnTarget(targetFile);
     * let res = await spApi.request.stream(req_options);
     * streamOnTargetFile(res).then(() => {
    ...
    streamDownload(req_options) {
      return new Promise((resolve, reject) => {
        // check if method is PUT
        if (req_options.method == 'PUT') {
          reject(new Error('Method must be not PUT'));
        }
        let url = new URL(req_options.url);
        let options = {
          method: req_options.method,
